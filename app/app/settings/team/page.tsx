...
import { assignProjectAccess, createTeamInvite } from "@/app/app/settings/team/actions";
...
export default async function TeamPage({ searchParams }: { searchParams: { error?: string; success?: string } }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <Card><CardHeader><CardTitle>Organization members</CardTitle></CardHeader><CardContent>{members.length ? <Table><TableHeader><TableRow><TableHead>Member</TableHead><TableHead>Role</TableHead><TableHead>Project access</TableHead><TableHead>Joined</TableHead></TableRow></TableHeader><TableBody>{members.map((member) => <TableRow key={member.id}><TableCell><p className="font-semibold">{member.user.name ?? "Unnamed user"}</p><p className="text-xs text-muted-foreground">{member.user.email}</p></TableCell><TableCell><Badge variant={member.role === "OWNER" ? "default" : "secondary"}>{enumLabel(member.role)}</Badge></TableCell><TableCell>{member.role === "SITE_ENGINEER" ? member.projectMembers.map((item) => item.project.name).join(", ") || "No projects assigned" : "Organization-wide as permitted"}</TableCell><TableCell>{shortDate(member.createdAt)}</TableCell></TableRow>)}</TableBody></Table> : <EmptyState icon={UsersRound} title="No team members" description="Invite teammates and assign the role that matches their responsibilities." />}</CardContent></Card>
