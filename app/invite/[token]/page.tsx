...
export default async function InvitePage({
  ...
  async function accept(formData: FormData) {
    "use server";
    const currentInvite = await prisma.organizationInvite.findUnique({ where: { token: params.token } });
    if (!currentInvite || currentInvite.acceptedAt || currentInvite.expiresAt <= new Date()) redirect(`/invite/${params.token}?error=This invite is invalid or expired.`);
    const name = String(formData.get("name") ?? "").trim();
    const password = String(formData.get("password") ?? "");
    if (name.length < 2 || password.length < 8) redirect(`/invite/${params.token}?error=Enter your name and a password of at least 8 characters.`);
    const existing = await prisma.user.findUnique({ where: { email: currentInvite.email } });
    if (existing) redirect(`/invite/${params.token}?error=An account already exists for this email. Ask the administrator for a fresh membership invite.`);
